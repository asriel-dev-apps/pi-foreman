...
| 無い・それ以外・git リポジトリの外 | 何も送らない。依頼文の射影（下記）を手元のルール表で判定する（決定 5 の結果） |
...
**結果（2026-09-23）**: 見ていない holdout で `full` 10/12、`facts` 5/12、`rules` 6/12
（語彙を作るとき見たフィクスチャでは 19/19、16/19、18/19）。射影を送っても jev はルール表に勝たないので、
...
`Jev: off` の repo では判定しないのでログも残さない。`Jev: full` で jev の呼び出しが失敗したときは `jev: null`・
`advice: []`（通常時もこのとき助言は出ない）で残す。ログは shadow でない通常時も同じ形で残し、`shadow` 欄で区別する。
